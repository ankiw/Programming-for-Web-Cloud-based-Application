...
function Game(tileNames) {
  ...
  this.flipTile = function(tile) {
    ...
    if (!this.firstPick || this.secondPick ||this.thirdflip) {

      if (this.secondPick) {
        this.firstPick.flip();
        this.secondPick.flip();
        this.firstPick = this.secondPick =this.thirdflip = undefined;
      }

      this.firstPick = tile;
      
      this.message = Game.MESSAGE_TWO_MORE;
...
Game.MESSAGE_TWO_MORE = 'Pick one more card.'
